...
const Load = (props) => {
  ...
  const fetchResults = async () => {
    getResults(props.accessToken).then((res) => setresults([res.data])).catch()
  }

  const fetchVersions = async () => {
    getVersions().then((res) => {setversions(res.data)}).catch(setversions({'A': 'Version 1'}))
  }
  ...
  return (
    <div style={{ width: "50%", paddingTop: 300, margin: "auto" }}>
      {state.isLoading ? (
        ...
        >
          {Object.keys(versions).map((versionKey, index) => {
            return(
            <MenuItem key={versionKey} value={versionKey}>{versions[versionKey]}</MenuItem>
            )
          })}
        </Select>
      </FormControl>
      ...
      {results ? (
        ...
        <Button onClick={fetchResults} style={{marginTop: 20}} variant='contained' color='primary'>Start Download Data</Button>
      )}
