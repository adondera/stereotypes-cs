...
const Child = (props) => {
    ...
    const onChangeLastName = (id) => (event) => {
        ...
        if(ValidateConsentInput(event.target.value)) newChildren[id].lastName = event.target.value;
        newChildren[id].isValid = newChildren[id].lastName.length > 0 && newChildren[id].firstName.length > 0
        props.component.setState({children: newChildren})
      }
    
    const onChangeFirstName = (id) => (event) => {
        var newChildren = props.component.state.children
        if (ValidateConsentInput(event.target.value)) newChildren[id].firstName = event.target.value;
        newChildren[id].isValid = newChildren[id].lastName.length > 0 && newChildren[id].firstName.length > 0
        props.component.setState({children: newChildren})
      }
