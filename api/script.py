...
from api.models import Category, Metacategory, Image, Question, \
    QuestionType, QuestionChoice, Ethnicity, Gender
...
def populate():
    ...
    for link in male_images:
        Image.create_image(link=link, description="image of a man",
                           attribute='man standing', c_id=c_male.id)

    for link in female_images:
        Image.create_image(link=link, description='image of a girl',
    ...
    for ethnicity in Ethnicity:
        QuestionChoice.create_choice(choice_num=ethnicity.value, q_id=mc_2.id, text=ethnicity.name)

    mc_3 = Question.create_question(q_type=QuestionType.mc_single_answer,
                                    text="What is your gender?")
    for gender in Gender:
        QuestionChoice.create_choice(choice_num=gender.value, q_id=mc_3.id, text=gender.name)
